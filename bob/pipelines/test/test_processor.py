...
def test_processor_pipeline_only_transform_delay():
    ...
    try:
        ...
        checkpoints = {"dummy1": os.path.join(candidate_path, "dummy1"),
                       "dummy2":  os.path.join(candidate_path, "dummy2")}

        pip      = [("dummy1", Dummy(offset=1)), ("dummy2", functools.partial(Dummy, offset=2))]
        pipeline = ProcessorPipeline(pip)
...
def test_processor_pipeline_fit_transform():
    ...
    X_new_transform = pipeline.transform(sampleset_transform)
    assert numpy.allclose(X_new_transform[0].samples[0].data, numpy.zeros(shape=(10,2), dtype=int)+4)
...
def test_processor_pipeline_fit_transform_with_model_checkpoint():
    ...
    try:
        ...
        X_new_transform = new_pipeline.transform(sampleset_transform, checkpoints=checkpoints)
        assert numpy.allclose(X_new_transform[0].samples[0].data, numpy.zeros(shape=(10,2), dtype=int)+4)
    finally:
        shutil.rmtree(candidate_path)
...
def test_processor_pipeline_fit_transform_with_model_checkpoint_2():
    ...
    try:
        ...
        # Building a pipeline for fitting
        pip      = [("dummy1", Dummy(offset=1)), ("dummy2", DummyWithFit()), ("dummy3", DummyWithFit())]
        checkpoints = {"dummy2":  os.path.join(candidate_path, "dummy2")}    
        pipeline = ProcessorPipeline(pip)
        pipeline.fit(sampleset_fit, checkpoints=checkpoints)
        ...
        X_new_transform = pipeline.transform(sampleset_transform, checkpoints=checkpoints)
        assert numpy.allclose(X_new_transform[0].samples[0].data, numpy.zeros(shape=(10,2), dtype=int)+8)        
    finally:
        shutil.rmtree(candidate_path)
